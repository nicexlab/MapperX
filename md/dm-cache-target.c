...
struct cache {
	...
	unsigned long last_clear_cnt_jiffies;
...
static int set_abt(struct cache *cache, dm_oblock_t oblock, bool is_dirty)
{
	unsigned r;
	...
	if (get_cache_mode(cache) >= CM_READ_ONLY)
        ...
        if (is_dirty) {
		cache->write_cnt[oblock]++;
		cache->total_meta_write++;
	}
	else cache->write_cnt[oblock]--;
	r = dm_cache_set_big_dirty(cache->cmd, oblock, is_dirty);
	if (r) {
...
static inline void set_cbt_dirty(struct cache *cache, dm_oblock_t oblock)
{
	unsigned long shift;
	int i;
	//=======update cbt========
	for (i = 0; i <= LY; i++) {
		shift = (oblock >> i) + (1 << LY - i) - 1;
		cache->cbt[shift]++;
	}
}

static inline void clear_cbt_dirty(struct cache *cache, dm_oblock_t oblock)
{
	unsigned long shift;
	int i;
	//=======update cbt========
	...
	if (!test_and_set_bit(from_cblock(cblock), cache->dirty_bitset)) {
		...
		oblock /= 32;
                for (i = 0; i <= LY; i++) {
                        shift = (oblock >> i) + (1 << LY - i) - 1;
                        cache->cbt[shift]++;
                        if (cache->abt[shift] && cache->cbt[shift] == 1 && strstr(current->comm, "kwork")) {
				cache->ww++;
                                set_abt(cache, shift, true);
                        }
                }
	...
	if (test_and_clear_bit(from_cblock(cblock), cache->dirty_bitset)) {
		...
		oblock /= 32;
        	for (i = 0; i <= LY; i++) {
                	shift = (oblock >> i) + (1 << LY - i) - 1;
			cache->cbt[shift]--;
			if (cache->abt[shift] && cache->cbt[shift] == 0 && strstr(current->comm, "kwork")) {
				cache->ww++;
				set_abt(cache, shift, false);
			}
		}
...
static int need_clear_cnt_due_to_time(struct cache *cache)
{
	return jiffies < cache->last_clear_cnt_jiffies ||
	       jiffies > cache->last_clear_cnt_jiffies + COMMIT_PERIOD;
}


static void periodic_adjust(struct cache *cache){
	double x = (double)cache->total_meta_write / cache->total_write;
	double sla = 0.001;
	int i, ii;
	bool yes = cache->total_meta_write > sla * cache->total_write;
	for (ii = 0; ii < 5; ii++)
		if (cache->total_meta_write + cache->total_write > 0){
			if (yes) {
				int max_write = -1;
				int k = -1;
				for (i = 0; i < (1 << LY) - 1; i++)
					if (cache->abt[i + 1 << 1] && cache->abt[(i + 1 << 1) - 1] && cache->write_cnt[i] > max_write) {
						max_write = cache->write_cnt[i];
						k = i;
					}

				if (k != -1) {
					cache->abt[k] = true;
					cache->abt[k + 1 << 1] = false;
					cache->abt[(k + 1 << 1) - 1] = false;
					set_abt(cache, k, cache->cbt[k]);
					set_abt(cache, k + 1 << 1, false);
					set_abt(cache, (k + 1 << 1) - 1, false);
				}

			}
			else {
				int min_write = 1000000;
				int k = -1;
				for (i = 0; i < (1 << LY) - 1; i++)
					if (cache->abt[i] && cache->write_cnt[i] < min_write && cache->cbt[i] != 0) {
						min_write = cache->write_cnt[i];
						k = i;
					}
				if (k != -1) {
					cache->abt[k] = false;
					cache->abt[k + 1 << 1] = true;
					cache->abt[(k + 1 << 1) - 1] = true;
					set_abt(cache, k, false);
					set_abt(cache, k + 1 << 1, cache->cbt[k + 1 << 1]);
					set_abt(cache, (k + 1 << 1) - 1, cache->cbt[(k + 1 << 1) - 1]);
				}
	...
	cache->k += s2;
	cache->k2++;
	double recover_time = 1 / 8.0  * ss;
	cache->k1 += recover_time;
	cache->last_clear_cnt_jiffies = jiffies;
	cache->total_write = 0;
	cache->total_meta_write = 0;
}
...
static void do_worker(struct work_struct *ws)
{
	...
	do {
		if (need_clear_cnt_due_to_time(cache)){
			cache->last_clear_cnt_jiffies = jiffies;
			periodic_adjust(cache);
		}	
		if (!is_quiescing(cache)) {
...
static int cache_create(struct cache_args *ca, struct cache **result)
{
	...
	cache->write_cnt = vmalloc(sizeof(unsigned long) * (1 << LY + 1));
	cache->cbt = vmalloc(sizeof(unsigned long) * (1 << LY + 1));
	cache->abt = vmalloc(sizeof(bool) * (1 << LY + 1));
	...
	clear_big_bitset(cache->write_cnt, from_cblock(1 << LY + 1));
	clear_big_bitset(cache->cbt, from_cblock(1 << LY + 1));
	clear_cbt(cache->abt, sizeof(bool) * (1 << LY + 1));
	cache->abt[0] = true;
